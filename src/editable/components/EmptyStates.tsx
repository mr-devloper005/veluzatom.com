...
export function EmptyState({
  ...
  return (
    <section className={cn('rounded-[2rem] border border-black/8 bg-white p-8 text-center shadow-[0_12px_40px_rgba(24,23,19,0.08)]', className)}>
      <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[var(--slot4-page-bg)]">
        <SearchX className="h-6 w-6" />
      </div>
      <h2 className="mt-5 text-2xl font-black uppercase tracking-[-0.03em]">{title}</h2>
      <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-black/65">{description}</p>
      <Link href={actionHref} className="mt-6 inline-flex items-center gap-2 rounded-full border border-black/10 px-5 py-3 text-sm font-black uppercase tracking-[0.14em] transition hover:bg-black hover:text-white">
        {actionLabel}
